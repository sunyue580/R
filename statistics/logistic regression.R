...
exp(coef(fit_step))
#结果解释：
#1、二分类变量      OR表示其他变量不变的情况下，赋值为1相对比0的优势比情况，及发生结局事件的风险1是0的多少倍
#2、等级变量        OR表示其他变量不变的情况下，x增加一个等级时的优势比
#3、连续变量        OR表示其他变量不变的情况下，x增加一个等级时的优势比x增加1个单位(如1岁)时的优势比
#4、多分类变量      如有四种血型，分别用1，2，3，4表示，需将3个变量设为哑变量或指示变量
